...
- **中文搜索**：jieba 分词（词典 CDN/fetch 加载 + sha256 校验 + OPFS 缓存）；断网降级 bigram 不报错。
...
- `dict.bin` + `sha256_prefix.bin` — jieba 词典（运行时 fetch，非内联编译）
...
- **词典三渠道**：`dictData` 内联 / `dictUrl` CDN fetch + sha256 校验 + OPFS 缓存（二次零网络）/ 降级 bigram + warn。

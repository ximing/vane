...
日期：2026-08-21（v1.1：两份评审意见闭环修订；v1.2：复审遗留 1 条矛盾 + 3 条建议修正）
...
上游依据：维护者验收反馈（2026-08-21），逐条对应本文 §2–§6。
...
- 本规范冻结条件：~~复审通过 + 开发计划评审通过~~ 复审已通过（2026-08-21，v1.2 起冻结；行为条款不得再改，zh 文案与实现细节允许在开发计划评审中微调）。
